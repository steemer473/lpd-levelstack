...
**Status:** **Shipped 2026-07-19 (schema only)** — Zod in [`lib/pipeline/recommendation-types.ts`](../../lib/pipeline/recommendation-types.ts); optional `recommendations[]` on `report_json`; migration path [`docs/plans/recommendation-object.md`](./recommendation-object.md) + [`map-to-recommendation.ts`](../../lib/pipeline/map-to-recommendation.ts). Section rebuild = P2-4; Roadmap UI = P2-5.

**Acceptance criteria:**

- A schema exists (Zod types minimum, plus persistence as richer nested schema inside existing `report_json` JSONB per OD-3 — not new relational recommendation/evidence tables in V1) with fields for: structured evidence (not just prose), confidence (value + methodology reference), priority, ROI, a dependency graph (not a loose string match), a typed owner, and an automatability flag. — **Met.**
...
**Status:** **Locked 2026-07-19 (docs only)** — satisfied by [`docs/plans/evidence-provenance.md`](./evidence-provenance.md). No evidence-store table; provenance nests in `report_json` + existing SERP cache (OD-3 / OD-10).

**Acceptance criteria:**

- A documented standard exists specifying, at minimum: source identification, capture timestamp, and a staleness/expiry rule per evidence type (e.g., a GBP check from 90 days ago is not current evidence). — **Met:** `evidence-provenance.md` (source types, `capturedAt`, freshness classes, generation-time staleness table).
- The standard is referenced by, and precedes, the `evidence` field definition in P2-1's schema. — **Met:** this file + §5 `evidence` row; P2-1 Zod must match that shape.
...
**Status:** **Locked 2026-07-19 (docs only)** — satisfied by [`docs/plans/confidence-methodology.md`](./confidence-methodology.md). V1 = rule-based `Low` / `Medium` / `High` bands; no customer-facing %.

**Acceptance criteria:**

- A documented decision exists on the confidence methodology, with rationale for what's credible to ship in V1 (given no outcome data exists yet to validate historical-base-rate confidence) versus what has to wait. — **Met:** rule-based bands now; LLM self-score and historical base rates deferred to V2+.
- No confidence percentage renders in the product without a footnote/link to this methodology. — **Met for V1:** no % ships; bands + `methodologyRef` only.
- Per critique §4's caution on "false precision," if the methodology can't yet support numeric precision (e.g., LLM-judgment-based scoring on unstructured text), confidence should ship as a band (Low/Medium/High) with qualitative rationale rather than a fake-precise percentage. — **Met:** `band` + `rationale` + OD-14 intensity guardrail documented.
...
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `evidence`       | **Absent.** `findingSchema` has `value`/`detail` as informal prose; no structured evidence, no source/timestamp.                                                                                                                                                      | **Implemented** in `lib/pipeline/recommendation-types.ts` per [`evidence-provenance.md`](./evidence-provenance.md) (P2-2). Nested in optional `report_json.recommendations[]`; no evidence store table (OD-3/OD-10). |
| `confidence`     | **Absent.** No field anywhere in `findingSchema`/`actionItemSchema`. Unrelated `AuditSignalResult.evidence: string[]` exists but is pipeline-internal, not customer-facing.                                                                                           | **Implemented** per [`confidence-methodology.md`](./confidence-methodology.md) (P2-3): `band` + `rationale` + `methodologyRef`. Live band assignment = P2-4. |
| `priority`       | **Informally proxied**, not a real field. `severity` (critical/high/medium/low/good) on findings, plus a local, non-persisted sort `weight` in `buildActionPlanFromSections` (`action-plan.ts:25-33`).                                                                | **Implemented:** `P0`–`P3` on Recommendation Object (`recommendation-object.md`). |
| `dependencies`   | **No graph.** Closest analog is `findingRef`, a loose string match back to a finding by label text (`actionItemSchema`, `report-types.ts:66-79`).                                                                                                                     | **Implemented:** `{ recommendationIds: string[] }`. |
| `roi`            | **Absent.** No field, no calculation path.                                                                                                                                                                                                                            | **Implemented:** `{ kind, rangeLabel, notes? }` — ranges/labels only (no $ point estimates). |
| `automatability` | **Partially exists, narrowly scoped.** `automatorFlag`/`automatorProduct` (`"seo"|"workflow"`), computed by keyword regex `automatorMatch()` (`assemble-from-signals.ts:18-31`) — flags whether *LPD's own* Automator Pro products apply, not general automatability. | **Implemented:** `{ automatable, lpdProduct? }` — generalizes existing flag. |
| `owner`          | **Free-text only.** `who` field on `actionItemSchema` is free text like "You"/"Freelancer," not a typed identity.                                                                                                                                                     | **Implemented:** `{ role }` enum. |
| `artifact`       | **Absent — added by OD-13's resolution, not part of critique §8/§13's original field list.** No structured deliverable-content field exists on `findingSchema`/`actionItemSchema` today; this is the same gap `CURRENT_SPRINT_GOALS.md` #12 ("Report Value Slice 3") was closing separately before OD-13 folded it in.                                            | **Implemented:** `{ kind, content? }`. Populated in P2-4 for Search + Reputation. |
| `urgency`        | **Absent — added by OD-14's resolution.** No "why now" concept exists on `findingSchema`/`actionItemSchema` today; distinct from `roi` (value) and `priority` (rank).                                                                                               | **Implemented** as string field; intensity guardrail enforced at P2-4 synthesis. |
| `consequenceOfInaction` | **Absent — added by OD-14's resolution.** No inaction-risk field exists today.                                                                                                                                                                                | **Implemented** as string field; same OD-14 guardrail as `urgency`. |
